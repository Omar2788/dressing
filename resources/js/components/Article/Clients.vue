<template>
  <div class="row">
    ...
    <DataTable v-model:filters="filters" :value="reverseClient" :loading="isLoading" selectionMode="single" paginator rows="6" :rowsPerPageOptions="[6, 20, 50,100,200,500,1000]">
      <template #header>
    ...
    <div class="dialog">
        <div class="row">
          <div class="col-md-12">
            ...
            <p>Cette cliente a été ajouté le <b>{{ formatDate(selectedClient.created_at) }}</b> à <b>{{ formatTime(selectedClient.created_at) }}</b></p>
          </div>
        </div>
        <hr>
        <div class="row">
        ...
        <div class="row">
  <label v-if="selectedClient && selectedClient.image">Si tu veux changer limage de la cliente cliquer au dessous <i class="bi bi-arrow-down"></i></label>
  <img v-if="selectedClient && selectedClient.image" :src="selectedClient.image" alt="image" class="imageClientEdit" @click="isEditDialogVisible = true" />
...
<label v-if="selectedClient && selectedClient.image" for="changeImageCheckbox">Changer l'image de la cliente</label>
